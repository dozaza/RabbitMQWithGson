import com.dyuproject.protostuff.{Schema, ProtobufIOUtil}
...
class RmqThread extends Runnable {
  ...
  private def handleMessage(): Unit = {
    ...
    val schema = RuntimeSchema.getSchema(classOf[Stock_java])
    ...
    while(true)  {
      ...
      if(delivery != null) {
        ...
        if(routingKey == client.EQUITY_KEY) {
          ...
          client.count += 1
          val buff = decoder.decodeBuffer(message)
          val s = new Stock_java()
          ProtobufIOUtil.mergeFrom(buff, s, schema)
          println(s)
